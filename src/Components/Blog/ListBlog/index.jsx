...
const ArticleCard = ({ article }) => (
  <div className="border rounded-lg p-6 mb-8 shadow-md hover:shadow-xl transition-shadow duration-300 w-full max-w-4xl flex flex-col items-center bg-white">
    <img
      src={article.image}
      alt={article.title}
      className="w-full h-96 object-cover rounded-md"
    />
    <h2 className="text-2xl font-extrabold mt-5 text-center text-gray-900">
      {article.title}
    </h2>
    ...
    <div className="flex flex-wrap justify-between items-center mt-6 text-sm text-gray-500 w-full px-4">
      <NavLink
        to={"/Detail"}
      ...
      >
        ...
        <span className="text-base">ادامه مطلب</span>
      </NavLink>

      <div className="flex flex-wrap gap-6 ml-auto text-right text-gray-500">
        <span>
          تگ‌ها: <b>{article.tags.join(", ")}</b>
        </span>
        <span>
...
const ArticlesSection = () => {
  return (
    <div className="w-full flex flex-col items-center gap-10 py-8 bg-gray-50">
      {articles.slice(0, 2).map((article) => (
        <ArticleCard key={article.id} article={article} />
      ))}
    </div>
